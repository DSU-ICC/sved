document.addEventListener("DOMContentLoaded", () => {
    ...
    const showDisciplines = (disciplineList) => {
        ...
        let popupDeleteRpdBtns = document.querySelectorAll(".delete")
        popupDeleteRpdBtns.forEach(popupDeleteRpdBtn => {
            if (!popupDeleteRpdBtn.classList.contains("delete-discipline")) {
    ...
    const uploadFileRpd = async (formData, el) => {
        ...
        let response = await fetch(`${URL}/FileRPD/CreateRPD?disciplineId=${formData.get("disciplineId")}${ecpCode}`, {
            method: "POST",
            credentials: "include",
            body: formData
        })
    ...
    const deleteDiscipline = async (disciplineId, el) => {
        ...
        if (response.ok) {
            alert("Дисциплина успешно удален")
            el.classList.remove("loading")
            el.disabled = false
